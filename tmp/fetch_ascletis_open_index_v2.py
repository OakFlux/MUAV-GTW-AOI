...
def doh_ips() -> list[str]:
    ...
    for endpoint in endpoints:
        try:
            ...
            print('DOH',endpoint,r.status_code,len(r.content))
            obj=r.json()
            for ans in obj.get('Answer') or []:
                data=str(ans.get('data') or '')
                if re.fullmatch(r'\d{1,3}(?:\.\d{1,3}){3}',data): ips.append(data)
        except Exception as exc: print('DOH_ERR',endpoint,repr(exc))
...
ips=doh_ips()
print('IPS',ips)
if not ips: raise RuntimeError('No A records from public DoH')
...
# Directory listing is normally UTF-8, but try common fallbacks.
index=''
for encoding in ['utf-8','gb18030','latin1']:
    try:
        index=raw.decode(encoding)
        if 'Index of' in index or '<a ' in index: break
...
# Raw filename-context search, useful if browser display text is truncated.
for key in keys:
...
for item in links: print('MATCH',json.dumps(item,ensure_ascii=False))
...
for idx,item in enumerate(links,1):
    ...
    try:
        ...
        if sha in seen: out.unlink();continue
        seen.add(sha)
        final=PDFS/f'{idx:02d}_{sha[:12]}.pdf';out.rename(final)
        ...
        valid.append(meta);print('VALID',json.dumps({k:v for k,v in meta.items() if k!='sample'},ensure_ascii=False))
    except Exception as exc:
        print('VALIDATE_ERR',item['url'],repr(exc));out.unlink(missing_ok=True)
