...
const seeds = [
  ...
  'http://www.sinofert.com/sinoferten/index.html',
...
const results = [];
const interesting = /invest|report|annual|interim|announcement|financial|download|filing|hkex|公告|報告|报告|年報|年报|中期|投資者|投资者|下載|下载/i;
...
while (queue.length && visited.size < 100) {
  ...
  try {
    ...
    const links = await page.evaluate(() => [...document.querySelectorAll('a')].map(a => ({
      ...
      parent:(a.closest('tr,li,article,section,div')?.innerText||'').replace(/\s+/g,' ').trim().slice(0,1000),
    ...
    results.push({url, depth, status:res?.status()||null, title, body, pdfs, links: normalized.filter(x=>interesting.test(`${x.text} ${x.title} ${x.parent} ${x.url}`)).slice(0,500)});
    for (const x of normalized) {
      if (depth >= 2) break;
      let u;
      try { u = new URL(x.url); } catch { continue; }
      const same = /(^|\.)sinofert\.com$/i.test(u.hostname);
      if (same && interesting.test(`${x.text} ${x.title} ${x.parent} ${x.url}`) && !visited.has(x.url)) queue.push({url:x.url, depth:depth+1});
    }
  } catch (e) {
...
for (const r of results) {
  for (const p of (r.pdfs||[])) console.log('PDF',r.url,'::',p.text,'::',p.parent,'::',p.url);
  for (const l of (r.links||[])) if (/年報|年报|annual report|中期業績|中期业绩|interim results/i.test(`${l.text} ${l.title} ${l.parent}`)) console.log('KEY',r.url,'::',l.text,'::',l.parent,'::',l.url);
}
